...
{
    printf("Reflection over an axis with an inclination angle of %g degrees\n",
...
float **s_matrix(var *ptr, int ac, char **av)
{
    ...
    ptr->matrix[0][0] = cos(2 * ptr->angle);
    ptr->matrix[0][1] = sin(2 * ptr->angle);
    ptr->matrix[0][2] = 0;
    ptr->matrix[1][0] = sin(2 * ptr->angle);
    ptr->matrix[1][1] = -cos(2 * ptr->angle);
    ptr->matrix[1][2] = 0;
    ptr->matrix[2][0] = 0;
    ptr->matrix[2][1] = 0;
    ptr->matrix[2][2] = 1;
...
float **reflection_axis(int ac, char**av)
{
    ...
    vector[0], vector[1], multiplicate[0], multiplicate[1]);
        return ptr->matrix;
    } else if (ac != 5 || av[3][0] != '-' || av[3][1] != 's' || check_numbers(av[1]) == 84 ||
